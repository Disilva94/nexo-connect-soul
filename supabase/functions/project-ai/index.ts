...
serve(async (req) => {
  ...
  try {
    ...
    const [projectRes, tasksRes, risksRes, docsRes] = await Promise.all([
      userClient.from("projects").select("id,name,objective,description,status,health,progress,health_reason,budget_planned,budget_actual,start_date,end_date").eq("id", project_id).single(),
    ...
    const answer = [
      ...
      "Sugestões geradas pela IA: priorize itens atrasados, revise riscos críticos sem ação preventiva e confirme os próximos marcos antes do próximo checkpoint.",
