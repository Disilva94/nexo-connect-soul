...
function SmartProjectWizard({ defaultOrgId }: { defaultOrgId?: string }) {
  ...
  async function createProjectWithStructure() {
    ...
    const { error: memberError } = await db.rpc("ensure_current_user_project_owner_member", { _project_id: projectId });
    if (memberError) warnings.push(`vínculo do criador ao projeto: ${memberError.message}`);
